...
def get_train_data_loaders(cfg):
    ...
    num_source_train = round(num_max_iter*cfg.TRAINER.BATCH_SIZE[0]) if cfg.TRAINER.COPY_LIST[0] == 'Y' else num_source
    num_target_train = round(num_max_iter*cfg.TRAINER.BATCH_SIZE[1]) if cfg.TRAINER.COPY_LIST[1] == 'Y' else num_target
    ...
    source_set = TSNDataSet(cfg.PATHS.PATH_DATA_SOURCE+".pkl", cfg.PATHS.TRAIN_SOURCE_LIST, num_dataload=num_source_train, num_segments=cfg.DATASET.NUM_SEGMENTS,
    ...
    source_loader = torch.utils.data.DataLoader(source_set, batch_size=cfg.TRAINER.BATCH_SIZE[0], shuffle=False, sampler=source_sampler, num_workers=cfg.TRAINER.JOBS, pin_memory=True)

    target_set = TSNDataSet(cfg.PATHS.PATH_DATA_TARGET+".pkl", cfg.PATHS.TRAIN_TARGET_LIST, num_dataload=num_target_train, num_segments=cfg.DATASET.NUM_SEGMENTS,
                    new_length=data_length, modality=cfg.DATASET.MODALITY,
                    image_tmpl="img_{:05d}.t7" if cfg.DATASET.MODALITY in ["RGB", "RGBDiff", "RGBDiff2", "RGBDiffplus"] else cfg.MODEL.FLOW_PREFIX + "{}_{:05d}.t7",
                    random_shift=False,
                    test_mode=True,
    ...
    target_loader = torch.utils.data.DataLoader(target_set, batch_size=cfg.TRAINER.BATCH_SIZE[1], shuffle=False, sampler=target_sampler, num_workers=cfg.TRAINER.JOBS, pin_memory=True)
...
def get_val_data_loaders(cfg):
    ...
    source_set_val = TSNDataSet(cfg.PATHS.PATH_VAL_DATA_SOURCE+".pkl", cfg.PATHS.VAL_SOURCE_LIST, num_dataload=num_source_val, num_segments=cfg.DATASET.VAL_SEGMENTS,
                    new_length=data_length, modality=cfg.DATASET.MODALITY,
                    image_tmpl="img_{:05d}.t7" if cfg.DATASET.MODALITY in ["RGB", "RGBDiff", "RGBDiff2", "RGBDiffplus"] else cfg.MODEL.FLOW_PREFIX+"{}_{:05d}.t7",
                    random_shift=False,
                    test_mode=True,
    ...
    target_loader_val = torch.utils.data.DataLoader(target_set_val, batch_size=cfg.TRAINER.BATCH_SIZE[1], shuffle=False, sampler=target_sampler_val, num_workers=cfg.TRAINER.JOBS, pin_memory=True)
...
def get_test_data_loaders(cfg):
    ...
    if cfg.TESTER.NOUN_TARGET_DATA is not None:
        data_set = TSNDataSet(cfg.TESTER.TEST_TARGET_DATA+".pkl", cfg.PATHS.VAL_LIST, num_dataload=num_test, num_segments=cfg.TESTER.TEST_SEGMENTS,
            new_length=data_length, modality=cfg.DATASET.MODALITY,
            image_tmpl="img_{:05d}.t7" if cfg.DATASET.MODALITY in ['RGB', 'RGBDiff', 'RGBDiff2', 'RGBDiffplus'] else cfg.MODEL.FLOW_PREFIX+"{}_{:05d}.t7",
    ...
    data_loader = torch.utils.data.DataLoader(data_set, batch_size=cfg.TESTER.BATCH_SIZE, shuffle=False, num_workers=cfg.TRAINER.JOBS, pin_memory=True)
